...
LAPPDDecodeRawWaveform::LAPPDDecodeRawWaveform():Tool(){}


bool LAPPDDecodeRawWaveform::Initialise(std::string configfile, DataModel &data){
    /////////////////// Useful header ///////////////////////
    ...
    //Verbosity
    m_variables.Get("LAPPDDecodeRawWaveformVerbosity",LAPPDDecodeRawWaveformVerbosity);
    ...
    m_variables.Get("NUM_VECTOR_DATA1",NUM_VECTOR_DATA1);
    m_variables.Get("NUM_VECTOR_PPS1",NUM_VECTOR_PPS1);
...
//    if(LAPPDDecodeRawWaveformVerbosity>0) m_data->Stores["LAPPDData"]->Print(false);
//*/

    //Grab the amount of total entries in the LAPPDData Store
    m_data->Stores["ANNIEEvent"]->Header->Get("TotalEntries",entries);
    if(LAPPDDecodeRawWaveformVerbosity>0) cout << NewFileName << " got " << entries << endl;
    ...
    if(DoPedSubtract==1)//extra work for multi ped files and stores
    {
        ...
        if(ret)
        {
            ...
            if(LAPPDDecodeRawWaveformVerbosity>0) cout << PedFileName << " got " << Pedentries << endl;
    ...
    if(DoPedSubtract==1 && LAPPDDecodeRawWaveformVerbosity>1) cout<<"PEDSIZES: "<<PedestalValues->size()<<" "<<PedestalValues->at(0).size()<<" "<<PedestalValues->at(4).at(5)<<endl;
...
bool LAPPDDecodeRawWaveform::Execute(){

    //if((long)eventNo==entries) {std::cout<<"here"<<std::endl;m_data->vars.Set("StopLoop",1);}

    if(eventNo%10==0) cout<<"Event: " << eventNo << endl;

    PsecData dat;
    int i_entry = eventNo;
    if(LAPPDDecodeRawWaveformVerbosity>2) cout << "Starting entry " << i_entry << endl;
    //m_data->Stores["ANNIEEvent"]->GetEntry(i_entry);
    m_data->Stores["ANNIEEvent"]->Get("LAPPDData",dat);
    if(LAPPDDecodeRawWaveformVerbosity>2) cout << "Got entry " << i_entry << endl;
    ...
    if(LAPPDDecodeRawWaveformVerbosity>2) cout << "Number of boards was " << ReadBoards.size() << endl;

    int frametype = Raw_buffer.size()/ReadBoards.size();
    //cout<<"FRAMETYPE: "<<frametype<<endl;
    if(frametype!=NUM_VECTOR_DATA1 && frametype!=NUM_VECTOR_PPS1)
    {
        cout << "Problem identifying the frametype, size of raw vector was " << Raw_buffer.size() << endl;
        cout << "It was expected to be either " << NUM_VECTOR_DATA1*ReadBoards.size() << " or " <<  NUM_VECTOR_PPS1*ReadBoards.size() << endl;
        cout << "Please check manually!" << endl;
        return false;
    }

    if(frametype==NUM_VECTOR_PPS1)
    {
        if(LAPPDDecodeRawWaveformVerbosity>2) std::cout << "PPS Frame was read! Please fix me!" << std::endl;
        //return all as is
        /*
        ...
        */
        return true;
    }else if(frametype==NUM_VECTOR_DATA1)
    {
        if(LAPPDDecodeRawWaveformVerbosity>1) std::cout << "Data Frame was read! Starting the parsing!" << std::endl;
    }
    ...
    {
        Parse_buffer.clear();
        if(LAPPDDecodeRawWaveformVerbosity>2) std::cout << "Starting with board " << ReadBoards[bi] << std::endl;
        ...
        if(LAPPDDecodeRawWaveformVerbosity>2) std::cout << "Data for board " << ReadBoards[bi] << " was grabbed!" << std::endl;
        ...
        if(retval == 0)
        {
            if(LAPPDDecodeRawWaveformVerbosity>2) std::cout << "Data for board " << ReadBoards[bi] << " was parsed!" << std::endl;
            ...
            {
                if(LAPPDDecodeRawWaveformVerbosity>2) std::cout << "Meta for board " << ReadBoards[bi] << " was parsed!" << std::endl;
            }
        }else
        {
    ...
    if(LAPPDDecodeRawWaveformVerbosity>0) cout<<"BEGIN LAPPDDecodeRawWaveform "<< endl;
    ...
    if(LAPPDDecodeRawWaveformVerbosity>0) cout<<"*************************END LAPPDDecodeRawWaveform************************************"<<endl;
...
bool LAPPDDecodeRawWaveform::Finalise(){
...
bool LAPPDDecodeRawWaveform::ReadPedestals(int boardNo){

    if(LAPPDDecodeRawWaveformVerbosity>0) cout<<"Getting Pedestals "<<boardNo<<endl;
    ...
    if(LAPPDDecodeRawWaveformVerbosity>0) cout<<"Opened file: "<<LoadName<<endl;
    ...
    if(LAPPDDecodeRawWaveformVerbosity>0) cout<<"FINAL SAMPLE NUMBER: "<<PedestalValues->size()<<" "<<(((PedestalValues->find(0))->second)).size()<<endl;
    PedFile.close();
  return true;
}


bool LAPPDDecodeRawWaveform::MakePedestals(){
...
int LAPPDDecodeRawWaveform::getParsedMeta(std::vector<unsigned short> buffer, int BoardId)
{
    //Catch empty buffers
    if(buffer.size() == 0)
    {
    ...
    meta.push_back(std::to_string(BoardId));
    for(int CHIP=0; CHIP<NUM_PSEC; CHIP++)
    {
        meta.push_back((std::to_string(0xDCB0 | CHIP)));
        for(int INFOWORD=0; INFOWORD<13; INFOWORD++)
        {
            meta.push_back(std::to_string(PsecInfo[CHIP][INFOWORD]));
        }
        for(int TRIGGERWORD=0; TRIGGERWORD<6; TRIGGERWORD++)
        {
            meta.push_back(std::to_string(PsecTriggerInfo[CHIP][TRIGGERWORD]));
        }
    }

    meta.push_back(std::to_string(CombinedTriggerRateCount));
    meta.push_back(std::to_string(0xeeee));
    return 0;
}


int LAPPDDecodeRawWaveform::getParsedData(std::vector<unsigned short> buffer, int ch_start)
{
    //Catch empty buffers
    if(buffer.size() == 0)
    {
